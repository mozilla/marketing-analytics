...
WHERE
  active_clients.predicted_clv_12_months >= quantiles.q95_predicted_ltv_12_months
GROUP BY
  country
ORDER BY
  country
WITH
  active_clients AS (
    ...
    WHERE
      ...
      -- AND activity_group = 'aDAU'
      -- AND DATE_DIFF(DATE(2018, 10, 8), parse_date('%Y-%m-%d', max_activity_date), DAY) < 28
      AND DATE_DIFF(DATE(2018, 10, 8), parse_date('%Y-%m-%d', submission_date_s3), DAY) <= 28
      AND DATE_DIFF(DATE(2018, 10, 8), parse_date('%Y%m%d', max_submission_date_s3), DAY) <= 28
      -- in US, Canada, and Germany
      AND country IN ('US', 'CA', 'DE')
      -- with predicted_clv_12_months outliers removed
      AND predicted_clv_12_months < (
