...
SELECT
  sync_configured, avg(total_clv) avg_LTV, sum(predicted_clv_12_months)*100 sum_pltv, sum(total_clv)*100 sum_LTV, count(distinct(client_id)) n, count(distinct(client_id))*100 population
FROM
  `ltv.ltv_v1`
WHERE
 country = 'US'
