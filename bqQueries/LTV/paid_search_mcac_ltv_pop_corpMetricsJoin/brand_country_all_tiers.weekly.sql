...
WITH
  ...
  WHERE
    ...
    AND PARSE_DATE('%Y%m%d', _TABLE_SUFFIX) BETWEEN DATE(2019, 1, 1) AND DATE(2019,9,27)
    AND hits.type = 'EVENT'
  ...
  WHERE
    ...
    AND submission BETWEEN DATE(2019, 1, 1) AND DATE(2019,9,27)
  GROUP BY
    installsDate,
    content),
   ...
   FROM
    `ltv_v4_test.clients_calc_details_join`
   GROUP BY
    content),
