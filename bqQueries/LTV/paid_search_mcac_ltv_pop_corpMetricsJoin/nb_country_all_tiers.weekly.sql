-- used to populate Paid Search Non-brand Country Weekly
...
WITH
  ...
  WHERE
    ...
    AND PARSE_DATE('%Y%m%d', _TABLE_SUFFIX) BETWEEN DATE(2019, 1, 1) AND DATE(2019,3,13)
    AND hits.type = 'EVENT'
  ...
  WHERE
    ...
    AND PARSE_DATE('%Y%m%d', submission_date_s3) BETWEEN DATE(2019, 1, 1) AND DATE(2019,3,13)
  GROUP BY
    installsDate,
    content),
