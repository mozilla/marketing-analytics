...
SELECT
  ...
  DAU,
  aDAU,
  ROUND(AVG(DAU) OVER (ORDER BY funnelOrigin, submissionDate ROWS BETWEEN 27 PRECEDING AND CURRENT ROW),0) AS DAU28Day,
  ROUND(AVG(aDAU) OVER (ORDER BY funnelOrigin, submissionDate ROWS BETWEEN 27 PRECEDING AND CURRENT ROW),0) AS aDAU28Day,
  ROUND(AVG(DAU) OVER (ORDER BY funnelOrigin, submissionDate ROWS BETWEEN 90 PRECEDING AND CURRENT ROW),0) AS DAU91Day,
  ROUND(AVG(aDAU) OVER (ORDER BY funnelOrigin, submissionDate ROWS BETWEEN 90 PRECEDING AND CURRENT ROW),0) AS aDAU91Day
FROM data
GROUP BY 1,2,3,4