...
# testMetrics = daily summary
    ...
    joinedData = aggPageviews.join(currentYearAcquisitions, col('aggPageviews.client_id') == col('currentYearAcquisitions.np_clientId'), 'left_outer')
    # Remove duplication on join
    joinedData = joinedData.groupBy('submission_date_s3', 'client_id', 'totalURI', 'acqSegment').agg(min(joinedData.installDate).alias('installDate'))
