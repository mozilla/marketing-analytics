...
startPeriod = datetime(year=2018, month=9, day=3)
endPeriod = datetime(year=2018, month=9, day=8)
...
if period.days <= 30:
    ...
    metrics = metrics.filter(
        ...
        'searches')
    metrics = metrics.select('submission_date_s3', 'source', 'medium', 'campaign', 'content', upper(col('country')).alias('country'), 'DAU', 'activeDAU', 'totalURI', 'searches')
    ...
    installs.coalesce(1).write.option("header", "true").csv(
            's3://net-mozaws-prod-us-west-2-pipeline-analysis/gkabbz/weeklyReporting/Dimensioned/installs{}-{}.csv'.format(startPeriodString, endPeriodString))
    print("installs{}-{} completed and saved".format(startPeriodString, endPeriodString))
